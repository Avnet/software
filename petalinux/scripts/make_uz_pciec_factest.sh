...
# 
#  Description:         Build Script for UltraZed PCIe Carrier PetaLinux FAT
# 
#  Dependencies:        None
#
#  Revision:            Dec 30, 2016: 1.00 Initial version
#                       Feb 06, 2017: 1.01 Updated to include 
...
{ 
  ...
  echo " "
  if [ -f ${START_FOLDER}/${PETALINUX_CONFIGS_FOLDER}/config.UZ3EG_PCIEC_FACTEST ] 
    then
    cp -rf ${START_FOLDER}/${PETALINUX_CONFIGS_FOLDER}/config.UZ3EG_PCIEC_FACTEST \
  ...
  petalinux-create --type apps --name uz_pciec_factest --enable

  # Copy the uz_iocc_factest folder over to the uz_iocc_factest application
  # folder.
  cp -rf ${START_FOLDER}/${PETALINUX_APPS_FOLDER}/uz_pciec_factest/* \
  ${START_FOLDER}/${PETALINUX_PROJECTS_FOLDER}/${PETALINUX_PROJECT_NAME}/components/apps/uz_pciec_factest/
  ...
  echo " "
  cp -rf ${START_FOLDER}/${PETALINUX_CONFIGS_FOLDER}/rootfs/config.UZ3EG_PCIEC_FACTEST \

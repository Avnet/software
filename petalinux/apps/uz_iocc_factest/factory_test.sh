...
#  Project Name:        UltraZed I/O Carrier HW Factory Acceptance Test Script
#  Target Devices:      Xilinx Zynq UltraScale MPSoC
#  Hardware Boards:     UltraZed + I/O Carrier
# 
#  Tool versions:       Xilinx Vivado 2016.2
# 
#  Description:         Startup Script for UltraZed IOCC HW Factory Acceptance 
#                       Test design
# 
#  Dependencies:        
#
#  Revision:            Nov 21, 2016: 1.0 Initial version
# 
# ----------------------------------------------------------------------------
#!/etc/sh

# FAT version information.
FAT_VERSION_NUMBER=1.2
...
# devices.
CARRIER_PART_NUMBER=AES-US1CAR-1
SOM_PART_NUMBER=AES-US1SOM-1
SOM_ONLY_TESTS=-1

# Information about where test applications are located and what they are named:
EEPROM_TEST_APP=linux_eeprom_test.sh
IIC_GPIO_EXPANDER_TEST_APP=linux_iic_gpio_expander_test.sh
...
launch_all_common_tests ()
{
  ...
  then
    ...
    echo " "
    echo "The File ${EEPROM_TEST_APP} Does Not Exist at this Directory, ${TEST_APP_FOLDER}"
    echo " "
  fi
...
launch_all_som_specific_tests ()
{
  ...
  then
    ...
    echo " "
    echo "The File ${IIC_GPIO_EXPANDER_TEST_APP} Does Not Exist at this Directory, ${TEST_APP_FOLDER}"
    echo " "
  fi
  ...
  # Launch the Carrier IIC SOM EEPROM test.
  if [ -e ${TEST_APP_FOLDER}/${EEPROM_TEST_APP} ]
  then
    if [ -f ${TEST_APP_FOLDER}/${EEPROM_TEST_APP} ]
    then
...
print_fat_results_som_specific_tests ()
{
  ...
  if [ $SOM_IIC_EEPROM_TEST_RESULT == "0" ];
  then

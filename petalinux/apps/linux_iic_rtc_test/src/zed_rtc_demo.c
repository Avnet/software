...
void RTC_Isr(void *InstancePtr)
{
   ...
   if (!ret)
   {
      printf("ERROR: Failed to read RTC\n\r");
      return;
   }
...
int zed_rtc_demo_status(zed_rtc_demo_t *pDemo)
{
   ...
   if (!ret)
   {
      printf("ERROR : Failed to read I2C slave device RTC\n\r");
      return -1;
   }

   printf("RTC: Maxim DS3231M RTC Detected\n\r");

   printf("RTC: Seconds    = 0x%02X\n\r", RegData[DS3231M_SECONDS]);
   printf("RTC: Minutes    = 0x%02X\n\r", RegData[DS3231M_MINUTES]);
   printf("RTC: Hour       = 0x%02X\n\r", RegData[DS3231M_HOUR]);
   printf("RTC: Date       = 0x%02X\n\r", RegData[DS3231M_DATE]);
   printf("RTC: Month      = 0x%02X\n\r", RegData[DS3231M_MONTH]);
   printf("RTC: Year       = 0x%02X\n\r", RegData[DS3231M_YEAR]);
   printf("RTC: A1 Seconds = 0x%02X\n\r", RegData[DS3231M_ALARM1_SECONDS]);
   printf("RTC: A1 Minutes = 0x%02X\n\r", RegData[DS3231M_ALARM1_MINUTES]);
   printf("RTC: A1 Hour    = 0x%02X\n\r", RegData[DS3231M_ALARM1_HOUR]);
   printf("RTC: A1 Date    = 0x%02X\n\r", RegData[DS3231M_ALARM1_DATE]);
   printf("RTC: A2 Minutes = 0x%02X\n\r", RegData[DS3231M_ALARM2_MINUTES]);
   printf("RTC: A2 Hour    = 0x%02X\n\r", RegData[DS3231M_ALARM2_HOUR]);
   printf("RTC: A2 Date    = 0x%02X\n\r", RegData[DS3231M_ALARM2_DATE]);
   printf("RTC: Control    = 0x%02X\n\r", RegData[DS3231M_CONTROL]);
   printf("RTC: Status     = 0x%02X\n\r", RegData[DS3231M_STATUS]);
   printf("RTC: Age Offset = 0x%02X\n\r", RegData[DS3231M_AGING_OFFSET]);
   printf("RTC: Temp MSB   = 0x%02X\n\r", RegData[DS3231M_TEMP_MSB]);
   printf("RTC: Temp LSB   = 0x%02X\n\r", RegData[DS3231M_TEMP_LSB]);
